"""Fine-tune DGNet encoder/projection with a classification head."""
...
class FineTuneTrainer:
    def __init__(self, cfg: TrainConfig) -> None:
        ...
        self.logger.info("Fine-tune started config=%s", json.dumps(self.cfg.to_dict(), ensure_ascii=False, default=str))
        ...
        params = [
            {"params": [p for p in self.model.parameters() if p.requires_grad], "lr": cfg.encoder_lr},
            {"params": self.classifier.parameters(), "lr": cfg.classifier_lr},
        ]
    ...
    def _make_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"finetune.{id(self)}")
    ...
    def _configure_trainable_parameters(self) -> None:
        # Classification fine-tuning updates encoder/projection by default and keeps
        # DGNet's degradation/decoder frozen unless --no-freeze-encoder is used for full model training.
        for parameter in self.model.parameters():
            parameter.requires_grad_(not self.cfg.freeze_encoder)
        for module in (self.model.encoder, self.model.projection):
            for parameter in module.parameters():
                parameter.requires_grad_(True)
        for module in (self.model.degradation, self.model.decoder):
            for parameter in module.parameters():
    ...
    def _load_resume(self, resume: str) -> None:
        ...
        if (ckpt.parent / "model.pt").exists():
            self.model = DGNet.load(ckpt.parent, map_location=self.device, verbose=False).to(self.device)
            self._configure_trainable_parameters()
    ...
    def train_step(self, batch: Any) -> dict[str, float]:
        ...
        self.model.train()
        self.model.degradation.eval()
        self.model.decoder.eval()
        self.classifier.train()
        logits = self.classifier(extract_features(self.model, images))
    ...
    def _record(self, metrics: dict[str, float], split: str) -> None:
        row = self._base_row(split)
        row.update(metrics)
        append_metrics_csv(self.csv_path, row, CLASS_COLUMNS)

    def _validate_record_plot(self, train_metrics: dict[str, float] | None = None) -> None:
        metrics = self.validate()
        if train_metrics:
            metrics = {**train_metrics, **metrics}
        self._record(metrics, "validation")
        ...
        self.logger.info("validation step=%d epoch=%d acc=%.6f", self.global_step, self.epoch, metrics.get("accuracy", math.nan))
    ...
    def run(self) -> None:
        try:
            ...
            plot_loss_curve(self.csv_path, self.output_dir / "loss_curve.png")
            plot_accuracy_curve(self.csv_path, self.output_dir / "accuracy.png")
            self.logger.info("Fine-tune completed output_dir=%s", self.output_dir)
        except Exception:
            self.logger.error("fine-tune failed\n%s", traceback.format_exc())
            raise
...
def main(argv: list[str] | None = None) -> None:
    parser = build_arg_parser(default_task="finetune")
