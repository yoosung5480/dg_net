...
OUTPUT_ROOT="${OUTPUT_ROOT:-output/hybrid}"
DEVICE="${DEVICE:-auto}"
NUM_WORKERS="${NUM_WORKERS:-4}"
MAX_SAMPLES="${MAX_SAMPLES:-5000}"
EVAL_MAX_BATCHES="${EVAL_MAX_BATCHES:-20}"

PRETRAIN_RUN_ID="case1-hybrid-dgnet-pretrain-${STAMP}"
LINEAR_RUN_ID="case1-hybrid-dgnet-linear-probe-${STAMP}"
...
CUDA_VISIBLE_DEVICES=0 python engine/dg_train.py \
  ...
  --run-name case1-hybrid-dgnet-pretrain \
  --seed 42 \
  "${COMMON_DATA_ARGS[@]}" \
  "${COMMON_MODEL_ARGS[@]}" \
  "${LOSS_ARGS[@]}" \
...
CUDA_VISIBLE_DEVICES=0 python engine/linear_probe_train.py \
  ...
  --run-name case1-hybrid-dgnet-linear-probe \
  --seed 42 \
  "${COMMON_DATA_ARGS[@]}" \
  "${COMMON_MODEL_ARGS[@]}" \
